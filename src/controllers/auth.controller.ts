  ...
  if (code) {
    ...
    try {
      ...
      if (user) {
        ...
        const { refreshToken: newToken } = await UserDao.updateRefreshToken(
          email,
          encodedToken
        );
        ...
        res.cookie("refresh_token", encodedToken, {
...
export const checkToken = async (cookieHeader: string, res: Response) => {
  const regExp = new RegExp(/(?<=refresh_token=)[^;]+(?!;)/);
  const refreshToken = regExp.exec(cookieHeader);
